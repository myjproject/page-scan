...
let lastAnalysis = "";
const DEFAULT_ANALYSIS_ENDPOINT = "http://localhost:3001/analyze-status-log";
...
analyzeBtn.addEventListener("click", async () => {
  ...
  try {
    ...
    const capture = await runCapture();
    renderCapture(capture);
    renderAnalysis("正在等待 LLM 返回分析结果...");
    downloadBtn.disabled = false;
    await chrome.storage.local.set({ analysisEndpoint: endpoint });
    ...
    if (!response.ok) {
      throw new Error(responseData.error || `分析请求失败：HTTP ${response.status}`);
    }

    lastAnalysis = responseData.llmAnalysis || JSON.stringify(responseData, null, 2);
    renderAnalysis(lastAnalysis);
    await chrome.storage.local.set({ lastAnalysis });
    setStatus(`分析完成，共提取 ${responseData.statusLogCount || 0} 条状态日志。`);
  } catch (error) {
    console.error(error);
    renderAnalysis(`分析失败：${error.message}`);
...
async function hydrateLastCapture() {
  const {
    ...
    analysisEndpoint,
  } = await chrome.storage.local.get(["lastCapture", "lastAnalysis", "analysisEndpoint"]);

  endpointInput.value = analysisEndpoint || DEFAULT_ANALYSIS_ENDPOINT;
